...
func NewConfig(configPath string) (*Config, error) {
	...
	// Passwords are never on disk; restore the docker-demo password in memory
	// so "Local Demo" connect works without retyping every launch.
	c.hydrateDemoPasswords()

	return c, nil
}

// Demo connections for make docker-up / docker-seed (localhost:5432).
func DefaultDemoConnections() []types.Connection {
	now := time.Now()
...
// EnsureDemoConnections seeds docker-demo instances when the config is empty.
func (c *Config) EnsureDemoConnections() error {
	c.mu.Lock()
	defer c.mu.Unlock()
	if len(c.Connections) > 0 {
		return nil
	}
	for _, demo := range DefaultDemoConnections() {
		demo.ID = c.nextID
		c.nextID++
		c.Connections = append(c.Connections, demo)
	}
	...
	// save() strips passwords; put demo password back in memory for this session.
	c.hydrateDemoPasswordsLocked()
	return nil
}

func (c *Config) hydrateDemoPasswords() {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.hydrateDemoPasswordsLocked()
}

func (c *Config) hydrateDemoPasswordsLocked() {
	for i := range c.Connections {
		if isLocalDemoConn(c.Connections[i]) && c.Connections[i].Password == "" {
			c.Connections[i].Password = "postgres"
		}
	}
}

func isLocalDemoConn(conn types.Connection) bool {
	host := conn.Host
